...
class Session:
    ...
    def loadUser(self):
        profileName = 'corey'  # Change to take an input or paramater
    ...
    def saveUser(self):
        ...
        with open(f"../users/{self.user.username}'s-Profile.json", 'w') as json_file:
            resultsJSON = self.user.expenses
            for expense in resultsJSON:
                results.append(json.dumps(expense.__dict__))
            self.user.expenses = results
